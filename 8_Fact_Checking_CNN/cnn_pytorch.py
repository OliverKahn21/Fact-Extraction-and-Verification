...
base_dir = ''
pos_file = os.path.join(base_dir, 'pos.txt')
neg_file = os.path.join(base_dir, 'neg.txt')
...
def test(model, test_data):
    ...
    for data, label in test_loader:
        ...
        y_true.extend(label.data)
    # a = y_true[0].item()
    print([i.item() for i in y_true])
    print(y_pred)
    tmp = [i.item() for i in y_true]
    test_acc = metrics.accuracy_score(tmp, y_pred)
    test_f1 = metrics.f1_score(tmp, y_pred, average='macro')
    print("Test accuracy: {0:>7.2%}, F1-Score: {1:>7.2%}".format(test_acc, test_f1))

    print("Precision, Recall and F1-Score...")
    print(metrics.classification_report(tmp, y_pred, target_names=['SUPPORTS', 'REFUTES']))

    print('Confusion Matrix...')
    cm = metrics.confusion_matrix(tmp, y_pred)
    print(cm)
...
def predict(text):
    ...
    output = model(text)
    if output[0][0] < 0.2 and output[0][1] < 0.2:
